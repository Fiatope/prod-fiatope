module HeroHeaderTagHelper
  def hero_header_tag(object, options = {}, image = nil, &block)
    ...
    content_tag :header, capture(&block),
      ...
      style: "background-image: linear-gradient(to bottom, rgba(255, 255, 255, 0), rgba(0, 0, 0, 0.6)), url(#{image}); background-repeat: no-repeat; background-position: center center; background-size: cover; background-color: #1a1a1a; min-height: 60dvh",
