class ProjectObserver < ActiveRecord::Observer
  ...
  def after_create(project)
    ...
    if !project.partner.blank?
      ...
      if !partner.email.blank?
        NotificationsMailer.project_created_partner(project, partner).deliver
      end
    end

    #NotificationsMailer.project_created(project).deliver
  end
...
def from_online_to_fraud_suspiscion(project)
  ...
  def from_online_to_canceled(project)
    project.notify_owner(:project_canceled)  
  end

  def from_online_to_tobefollowed(project)
  end
