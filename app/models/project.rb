class Project < ActiveRecord::Base
  ...
  delegate :display_status,
           ...
           :display_net_amount,
           :display_goal,
           :display_pledged_presale,
           :display_goal_presale,
           :pledged_presale,
           :progress_presale,
           :remaining_days,
           :progress_bar,
  ...
  catarse_auto_html_for field: :english, video_width: 720, video_height: 405
  ...
  scope :visible, -> { without_states(['draft', 'rejected', 'deleted', 'tobefollowed']) }
  scope :tobefollowed, ->{ with_state('tobefollowed') } 
  scope :lovemoney, -> { where("category_id = 22 ") }
  scope :challenge, -> { where("challenge = 1 ") }
  ...
  scope :with_contributions_confirmed_this_hour, -> {
    joins(:contributions).merge(Contribution.confirmed_this_hour).uniq
  }
  scope :with_contributions_canceled_this_hour, -> {
    joins(:contributions).merge(Contribution.canceled_this_hour).uniq
  }
  ...
  # Used in multistep crowdfunding creation step2
  attr_accessor :description, :how_knows_fiatope, :how_knows_crowdfunding, :already_did_crowdfunding

  # Used in multistep crowdfunding creation step3
  attr_accessor :how_many_contributors, :how_much_contribute, :contributors_from_where, :enrollment_strategy

  # icons from https://www.iconfinder.com/iconsets/2018-social-media-logotypes
  CROWDFUNDING_SOCIAL_NETWORKS = %w(facebook twitter instagram)

  CROWDFUNDING_SOCIAL_NETWORKS.each do |network|
    attr_accessor network, "#{network}_followers"
  end

  # Used in multistep crowdfunding creation step3
  attr_accessor :need_support, :join_follow_program, :organization, :organization_type, :organization_description, :organization_created_at, :organization_in_incubateur, :organization_funding
