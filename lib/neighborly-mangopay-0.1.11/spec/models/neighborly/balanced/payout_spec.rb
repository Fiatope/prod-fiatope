...
describe Neighborly::Mangopay::Payout do
  let(:project) do
    double('Project')
  end
  let(:neighborly_customer) do
    double('Neighborly::Mangopay::Customer').as_null_object
  end
  ...
  describe "completion" do
    ...
    context "when customer already has a bank account" do
      let(:bank_accounts) { [double('::Mangopay::BankAccount')] }
      ...
      context 'with unsuccessful credit' do
        before do
          neighborly_customer.
            stub(:credit).
            and_raise(Mangopay::Conflict.new({}))
        end

        it 'skips payout logging' do
          expect(::Payout).to_not receive(:create)
          subject.complete! rescue Mangopay::Conflict
        end
      end
    end
    ...
    context "when customer has no bank accounts" do
      ...
      it "skips credit call" do
        ...
        subject.complete! rescue Neighborly::Mangopay::Error
      end

      it "raises NoBankAccount exception" do
        expect {
          subject.complete!
        }.to raise_error(Neighborly::Mangopay::NoBankAccount)
      end
    end
  end
