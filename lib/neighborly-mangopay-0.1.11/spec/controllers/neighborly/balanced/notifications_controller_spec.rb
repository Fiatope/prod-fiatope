...
describe Neighborly::Mangopay::NotificationsController do
  routes { Neighborly::Mangopay::Engine.routes }
  let(:event) { double('Event') }

  shared_examples 'create action' do
    it 'saves a new event' do
      expect_any_instance_of(Neighborly::Mangopay::Event).to receive(:save)
      post :create, params
    end

    context 'with valid event' do
      before do
        Neighborly::Mangopay::Event.any_instance.stub(:valid?).and_return(true)
      end
    ...
    context 'with invalid event' do
      before do
        Neighborly::Mangopay::Event.any_instance.stub(:valid?).and_return(false)
      end
